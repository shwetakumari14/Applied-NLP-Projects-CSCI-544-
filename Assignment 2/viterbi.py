...
def viterbiDecoding(sentence):    
    maxCalculatedProbilities, calculatedTags = [], []
    ...
    for i in range(1, len(sentence)):
        ...
        for j in range(len(calculatedProbilities)):
            individualTagProbability = []
            for k in range(len(calculatedProbilities)):
                emmisionKey = uniqueTags[j] + " <-> " + word
                transmissionKey = uniqueTags[k] + " <-> " + uniqueTags[j]
                if emmisionKey not in emissionParam or transmissionKey not in transitionParam:
                    individualTagProbability.append(np.float64(0.0000001))
                else:
                    individualTagProbability.append(np.float64(emissionParam[emmisionKey]*transitionParam[transmissionKey]*maxCalculatedProbilities[-1][k]+0.0001))
            calculatedProbilities[j] = max(individualTagProbability)
        maxCalculatedProbilities.append(calculatedProbilities)
        maxProbabilityIdx = calculatedProbilities.index(max(calculatedProbilities))
        calculatedTags.append(uniqueTags[maxProbabilityIdx])
...
def findTestPOSTags():
    ...
    for i in range(len(sentencesList)):
        calculatedTags = viterbiDecoding(sentencesList[i])
    ...
    greedyTxtFile = open("viterbi.out",  "w+")
